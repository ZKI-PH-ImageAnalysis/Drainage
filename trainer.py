...
class Trainer():
    def __init__(self, train_dataloader, logger, writer,
                 device, pass_model_to_loss, grad_bound=5.0, loss_name=None):
        # setting trainer
        self.device = device
    ...
    def update_info(self, optimizer, logits, labels, loss):
        ...
        if self.loss_name == 'alpha_dl':
            #ignore drainage node in accuracy calculation
            num_classes = logits.size(1)
            filtered_logits = logits[:, :num_classes - 1]
            self.info[ACC] = batch_accuracy(filtered_logits, labels)
        else:
